...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Strict English filtering, en-IN prioritization & per-gender cache
    // =============================================================
    const VoiceOutputManager = (() => {
        ...
        function getOrSelectVoice(gender) {
            const targetGender = gender || interviewerGender || 'male';
        ...
        function loadVoices() {
            ...
            if (voices.length) {
                getOrSelectVoice(interviewerGender);
            }
        }
        ...
        function speak(text) {
            ...
            const voiceSetup = getOrSelectVoice(interviewerGender);
        ...
        function mute() {
            isMuted = true;
            voiceCache[interviewerGender] = null; // Clear cached voice for current gender on mute
        ...
        function unmute() {
            isMuted = false;
            voiceCache[interviewerGender] = null; // Re-select voice on next speak
            if (btnTts) {
                const iconEl = btnTts.querySelector('.ctrl-icon');
                if (iconEl) iconEl.textContent = '🔊';
                const labelEl = btnTts.querySelector('.ctrl-label');
